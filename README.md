...
![撮影写真（左）と slidewarp で自動補正した結果（右）の比較](docs/assets/before-after.png)

<sub>斜めから撮った暗い会場のスライド写真（左）を、検出＋台形補正＋シャープ化して正対した
画像（右）に整えます。上の例は公開用に匿名化したサンプル（`samples/`）です。</sub>

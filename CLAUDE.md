...
> [`docs/detection-rust.md`](docs/detection-rust.md) を正とする。** 以下は要点サマリのみ。

- 候補生成は3系統: `contour`（明度マスク＋Cannyの輪郭を自前 Douglas-Peucker で四角形化）/
  `hough`（Canny を明部bbox拡張ROIに限定→H/V分離→帯域層化→総当りで四角形化。はみ出し・
  オクルージョン対策の要）/ `minrect`（最後の緩いフォールバック、信頼度に係数 0.6）。
- 選択は `score_quad` の統合スコア（重み: area0.12 rect0.05 aspect0.06 contrast0.12
  fill0.20 edge0.25 cut0.20。`fill` は `fill_holes` 版マスクを使う）。
- ⚠ **sub-slide 誤り（スライドの一部だけ切り出す）対策の主役は cut と方向付き edge**（`edge_profile`
  が法線方向サンプリングで同時算出）。重みを触るときはこの2項の役割を崩さないこと（理屈は doc §4）。
- **上下辺リファイン（Rust `detect.rs` のみ。`refine_top_edge`/`refine_bottom_edge`）**: 上辺/下辺が
  暗いタイトル帯/下部ロゴを切っている場合に真の枠まで辺を延ばす後処理。**発火は帯にコンテンツが
  ある時だけ**（損失非対称＝内容損失ゼロな辺は動かさない、が回帰防止の要）。候補生成・スコアを
  変えないので回帰しにくい。発火条件・走査ロジックは doc §5。
- 台形補正の画角外は黒塗り（Rust は warp の default pixel = 黒）。
...
- `.github/workflows/release.yml` が **`vX.Y.Z` タグの push** で起動し、3ターゲット
  （linux musl静的 / macOS arm64 / macOS x86_64）のバイナリを tar.gz＋sha256 で
...
このプロジェクトのファイルベース作業項目チケットは `doc/tickets/` 以下に置く
（GitHub Issues とは別管理）。`ticket` プラグイン（`/ticket-create`, `/ticket-check`,
`/ticket-triage`, `/ticket-fix`）で管理する。チケットのスキーマとライフサイクルは
`doc/tickets/CLAUDE.md` を参照。
